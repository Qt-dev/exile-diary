# AI Support Docs

This folder holds the deeper guidance that should not live in `AGENTS.md`.

## Core Guides
- [model-routing.md](./model-routing.md): choose the right model for the task
- [playbook.md](./playbook.md): request templates, verification rules, milestone checklist

## Runbooks
- [native-module-abi-mismatch.md](./runbooks/native-module-abi-mismatch.md)
- [single-instance-user-data-conflicts.md](./runbooks/single-instance-user-data-conflicts.md)
- [gpu-startup-issues.md](./runbooks/gpu-startup-issues.md)
- [stale-dev-process-or-port-conflicts.md](./runbooks/stale-dev-process-or-port-conflicts.md)
- [sqlite-extension-runtime-path-issues.md](./runbooks/sqlite-extension-runtime-path-issues.md)
- [ggg-api-dns-issues.md](./runbooks/ggg-api-dns-issues.md)

## Context Packs
- [electron-startup-runtime.md](./context/electron-startup-runtime.md)
- [runtime-sidecar.md](./context/runtime-sidecar.md)
- [ocr-image-parsing.md](./context/ocr-image-parsing.md)
- [db-repositories.md](./context/db-repositories.md)
- [test-harnesses.md](./context/test-harnesses.md)
