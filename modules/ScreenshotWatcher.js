      ...
      try {
        ...
        image.write(
          path.join(
            ...
            '.temp_capture',
            filename + '.' + path.basename(file, '.png') + '.area.png'
          )
        );
        ...
        image2.write(
          path.join(
            app.getPath('userData'),
            '.temp_capture',
            filename + '.' + path.basename(file, '.png') + '.mods.png'
          )
        );
  ...
  while (!isDone && offset < image.bitmap.height) {
    ...
    for (let y = offset; y < batchSize + offset; y++) {
      ...
      const isEndOfBlackBackground =
        lastLines.length === endDetectionHeight - 2 &&
        blackPixels < detectionWidth &&
        bluePixels < 1 &&
        Math.min(...lastLines.map((line) => line.black)) === detectionWidth;
      const isTooFarAfterBlueText =
        lastLines.length === endDetectionHeight - 2 &&
        bluePixels < 1 &&
        Math.max(...lastLines.map((line) => line.blue)) === 0;
