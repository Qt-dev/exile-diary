...
class DatasetGenerator {
  ...
  private corruptCharacterAt(text: string, position: number): string {
    ...
    switch (corruptionType) {
      ...
      case 1: // Character addition (artifact)
        const artifact = this.ARTIFACT_CHARS[Math.floor(Math.random() * this.ARTIFACT_CHARS.length)];
  ...
  private generateDataset(size: number, name: string): TestData[] {
    ...
    console.log(`${name}: ${totalCorruptedCount}/${size} corrupted (${((totalCorruptedCount/size)*100).toFixed(1)}%), ${firstCharCorruptedCount} first-char corrupted`);
    return dataset;
  }
