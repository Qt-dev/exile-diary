...
const parseMod = (modString) => {
  ...
  for (let i = 0; i < markupMatches.length; i++) {
    ...
    divs.push(<span style={style} className={className}>{text}</span>);
  }
...
const getDivCardFlavourText = (rawData) => {
  ...
  logger.info(rawData.flavourText);
  ...
  return (
    <div className="Card">
      <div className='Card__Face'>
        <img src={`https://web.poecdn.com/image/divination-card/${rawData.artFilename}.png`} />
      </div>
      ...
      <div className="Card__Text">
        ...
        <div className="Card_Stack">{rawData.pickupStackSize || rawData.stackSize}/{rawData.maxStackSize}</div>
        <div className="Card__Text__Main">
          <div className="Card__Mods">
            {getMods(rawData)}
          </div>
          <Divider className='Card__Divider' />
          <div className="Card__FlavourText">
            {getDivCardFlavourText(rawData)}
          </div>
        </div>
      </div>
    </div>
