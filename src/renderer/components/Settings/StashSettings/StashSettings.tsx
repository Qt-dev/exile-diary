...
const StashTabRow = ({ stashTab, indentationLevel = 0 }) => {
  ...
  return (
    <>
      ...
      {isFolder &&
        stashTab.children.map((child: any) => (
          <ObservedStashTabRow
            stashTab={child}
            key={child.id}
  ...
  return (
    <div className="Stash-Settings">
      <div className="Stash-Settings__Header">
        <div>
          Select the stash tabs you want to track, The app will then start pulling stats from these
          periodically
        </div>
        <div>
          Unique and Maps stash tabs are disabled because such tabs take so much time that Exile
          Diary would not be able to be able to read these in time for proper stats aggregation.
        </div>
      </div>
      ...
      <List className="Stash-Settings__List">
        {store.stashTabs.map((stashTab: any) => (
          <ObservedStashTabRow stashTab={stashTab} key={stashTab.id} />
        ))}
      </List>
    </div>
  );
