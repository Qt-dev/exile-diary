    ...
    for (const stage of stages) {
      ...
      if (results.length > 0) {
        const scoredResults = results.map(result => {
          ...
          const baseConfidence = stage.type === 'exact' ? 0.95 : 
                                  stage.type === 'close' ? 0.8 : 0.6;
        ...
        if (results.length > 0) {
          const scoredResults = results.map(result => {
            ...
            return {
              ...
              confidence: this.calculateConfidence(query, target, originalMatch, stage.type, preprocessing.corruptionLevel),
              matchType: stage.type
            };
          });
