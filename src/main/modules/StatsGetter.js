...
async function get(char, league) {
  ...
  for (let j = 0; j < charList.length; j++) {
    ...
    for (let i = 0; i < allMaps.length; i++) {
      ...
      if (
        m.name === 'Laboratory' &&
        m.runinfo.heistRogues &&
        Object.keys(m.runinfo.heistRogues).length > 0
      ) {
        m.areaType = 'heist';
      }
      ...
      if (
        m.areaType === 'heist' &&
        m.runinfo.heistRogues &&
        Object.keys(m.runinfo.heistRogues).length > 1
      ) {
...
async function getUnrighteousTurnedToAsh(char, league) {
  ...
  return new Promise((resolve, reject) => {
    DB.get(
      ...
      ${
        league
          ? ` and leaguedates.league = '${league}' and events.id between leaguedates.start and leaguedates.end `
          : ''
      }
    `,
      (err, row) => {
  ...
  for (let j = 0; j < charList.length; j++) {
    ...
    let q = await new Promise((resolve, reject) => {
      DB.all(
        ...
        where mapruns.id = areainfo.id
        and areainfo.name = ?
        and json_extract(runinfo, '$.blightedMap') is ${
          areaType.startsWith('blightedMaps') ? ' not null ' : ' null '
        }
        ${
          areaType.startsWith('heist')
            ? "and json_extract(runinfo, '$.heistRogues') is not null"
            : ''
        }
        ${
          areaType.startsWith('grandHeist')
            ? "and json_array_length(json_extract(runinfo, '$.heistRogues')) > 1"
            : ''
        }
        and json_extract(runinfo, '$.ignored') is null
        ${league ? ` and leaguedates.league = '${league}' ` : ''}
    ...
    q.forEach((map) => {
      ...
      maps.push({
        ...
        id: map.id,
        time: Number(
          Utils.getRunningTime(map.firstevent, map.lastevent, 's', { useGrouping: false })
        ),
        gained: map.gained || 0,
        kills: map.kills || 0,
...
function mergeAreaStats(areaStats, map) {
  areaStats[map.areaType] = areaStats[map.areaType] || {
    count: 0,
    gained: 0,
    kills: 0,
    time: 0,
...
function getKillInfo(char, start, end) {
  ...
  return new Promise((resolve) => {
    let DB = require('./DB').getDB(char);
    DB.get(
      ` select count(1) as count from events where id between ? and ? and event_type = 'slain' `,
      [start, end],
      (err, deaths) => {
        if (err) {
          resolve(null);
...
function mergeRunInfo(totalStats, map) {
  ...
  if (info.shaperBattle) {
    ...
    for (let i = 0; i < shaperBattlePhases.length; i++) {
      if (shaperBattlePhases[i].endpoint) {
        ...
        let runningTime = Number(
          Utils.getRunningTime(info.shaperBattle[prev], info.shaperBattle[curr], 's', {
            useGrouping: false,
          })
        );
  ...
  if (info.labyrinth) {
    totalStats.labyrinth = totalStats.labyrinth || {
      started: 0,
      completed: 0,
      argusKills: 0,
      darkshrines: {},
  ...
  if (info.masters) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      ['beasts', 'redBeasts', 'yellowBeasts', 'incursions', 'sulphite', 'favourGained'].forEach(
        (elem) => {
      ...
      if (minfo.missionMap) {
        ...
        totalStats.masters[m].missionMaps[minfo.missionMap] =
          ++totalStats.masters[m].missionMaps[minfo.missionMap] || 1;
      }
    }
  }
  ...
  if (info.syndicate) {
    ...
    for (let i = 0; i < keys.length; i++) {
      let s = keys[i];
      totalStats.syndicate[s] = totalStats.syndicate[s] || {
        encounters: 0,
        kills: 0,
        killedBy: 0,
      ...
      if (sinfo.killedPlayer) {
        totalStats.syndicate[s].killedBy =
          (totalStats.syndicate[s].killedBy || 0) + sinfo.killedPlayer;
      }
  ...
  if (info.sirusBattle) {
    totalStats.sirusBattle = totalStats.sirusBattle || {
      started: 0,
      completed: 0,
      dieBeamsFired: 0,
      dieBeamKills: 0,
    ...
    if (info.sirusBattle.completed) {
      ...
      if (info.sirusBattle.finalPhaseStart) {
        let lastPhaseTime = Utils.getRunningTime(
          info.sirusBattle.finalPhaseStart,
          info.sirusBattle.completed,
          's',
          { useGrouping: false }
        );
  ...
  if (info.conqueror) {
    ...
    for (var conq in info.conqueror) {
      totalStats.conquerors[conq] = totalStats.conquerors[conq] || {
        encounters: 0,
        battles: 0,
        kills: 0,
        orbs: 0,
  ...
  if (info.heistRogues && Object.keys(info.heistRogues).length > 0) {
    let isNormalHeist;
    totalStats.heist = totalStats.heist || {
      heists: 0,
      heistsCompleted: 0,
      grandHeists: 0,
      rogues: {},
    ...
    rogues.forEach((r) => {
      totalStats.heist.rogues[r] = totalStats.heist.rogues[r] || {
        heists: 0,
        heistsCompleted: 0,
        grandHeists: 0,
      };
  ...
  if (info.ultimatum) {
    totalStats.ultimatum = totalStats.ultimatum || {
      encounters: 0,
      ignored: 0,
      started: 0,
      completed: 0,
...
async function getBigDrops(char, league) {
  ...
  for (let i = drops.length - 1; i > -1; i--) {
    ...
    if (item.value < divinePrices[date]) {
      ...
      item.divineValue =
        item.typeline === 'Divine Orb' ? item.stacksize : item.value / divinePrices[date];
    }
  }
