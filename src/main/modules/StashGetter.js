...
class StashGetter {
      ...
      this.leagueDB.get(
        ...
        (err, row) => {
          if (err) {
            ...
            logger.info(
              `Last retrieved full stash ${row.timestamp} is ${Number(diff).toFixed(2)} hours old`
            );
            resolve(diff);
          }
        }
          ...
          if (row.timestamp == -1) {
            ...
            this.DB.get(
              ...
              (err, maps) => {
                ...
                logger.info(
                  `${maps.count} map runs since last retrieved full stash (set to retrieve every ${limit})`
                );
  ...
  async get(interval = 10) {
    ...
    for (var i = 0; i < tabList.length; i++) {
      ...
      if (tabData === -1) {
        logger.info(
          `Failed to get data for tab ${t.name}, aborting stash retrieval - will try again later`
        );
        emitter.emit('scheduleNewStashCheck');
        return;
      }
    ...
    if (tabs.items.length > 0) {
      this.leagueDB.get(
        'select value, length(items) as len from stashes order by timestamp desc limit 1',
        async (err, row) => {
          if (err) {
            ...
            if (row && Number(tabs.value).toFixed(2) === Number(row.value).toFixed(2)) {
              logger.info(
                `No change in ${this.league} stash value (${Number(tabs.value).toFixed(
                  2
                )}) since last update`
              );
              ...
              this.leagueDB.run(
                ...
                [timestamp, rawdata, tabs.value],
                (err) => {
                  if (err) {
                    logger.info(
                      `Error inserting ${this.league} stash ${timestamp} with value ${tabs.value}: ${err}`
                    );
                    emitter.emit('scheduleNewStashCheck');
                  } else {
                    logger.info(
                      `Inserted ${this.league} stash ${timestamp} with value ${tabs.value}`
                    );
                    this.leagueDB.get(
                      ...
                      (err, row) => {
                        if (err) {
                          logger.info(
                            `Error getting previous ${this.league} stash before ${timestamp}: ${err}`
                          );
                          emitter.emit('scheduleNewStashCheck');
                        } else {
                          if (getFullStash) {
                            let change =
                              row && row.value ? Number(tabs.value - row.value).toFixed(2) : 'new';
