...
function getEvent(content: string, server: string) {
  ...
  if (content.startsWith(':')) {
    ...
    if (match) {
      const { verb, character, text } = match.groups as { verb: string, character: string, text: string };
...
const LogProcessor = {
  ...
  processGeneration: async (timestamp: string, line: string) => {
    ...
    logger.info(`Detected map generation: ${timestamp}, level: ${actualLevel}, areaName: ${areaName},areaId: ${areaId}, seed: ${seed}`);
    ...
    if(!area.isTown && !area.isHideout) {
      hasProcessed = await RunParser.tryProcess({ event: { timestamp, server: lastInstanceServer } });
    }
    // If there is a map run ongoing, we don't create a new one
    const hasOngoingMap = await RunParser.hasOngoingMapRun();
    if ((isFirstRun || hasProcessed || !hasOngoingMap) && !area.isTown && !area.isHideout && !area.isLabyrinthAirlock && !area.isLabyrinthBossArea) {
      ...
      logger.info(`New map run created for area ${areaId} with level ${actualLevel} and seed ${seed}. Run ID: ${runId}`);
  ...
  processOther: async (timestamp: string, line: string) => {
    ...
    if (event) {
      try {
        ...
        await DB.insertEvent({timestamp, event_type: event.type, event_text: event.text, server: lastInstanceServer});
        ...
        if (event.type === 'entered') {
          ...
          await InventoryGetter.getInventoryDiffs(timestamp).then(async (diff) => { // Async but we dont care about the result here
