...
const Endpoints = {
  character: ({ accountName }) => `https://www.pathofexile.com/character-window/get-characters?accountName=${encodeURIComponent(accountName)}`,
  skillTree: ({ accountName, league, characterName }) => `https://www.pathofexile.com/character-window/get-passive-skills?league=${league}&accountName=${encodeURIComponent(accountName)}&character=${encodeURIComponent(characterName)}`,
  inventory: ({ accountName, league, characterName }) => `https://www.pathofexile.com/character-window/get-items?league=${league}&accountName=${encodeURIComponent(accountName)}&character=${encodeURIComponent(characterName)}`
}
...
const APIManager = {
  getCurrentCharacter: async () => {
    ...
    const currentCharacter = characters.find((character) => character.name === activeProfile.characterName);
