...
(function ($) {
  ...
  var ts = ($.tablesorter = {
    ...
    setup: function (table, c) {
      ...
      if (ts.debug(c, 'core')) {
        console.log(
          'Overall initialization time:' + ts.benchmark($.data(table, 'startoveralltimer'))
        );
    ...
    buildHeaders: function (c) {
      ...
      c.$headers = $(
        $.map(c.$table.find(c.selectorHeaders), function (elem, index) {
          ...
          elem.column = column;
          tmp = ts.getOrder(
            ts.getData($elem, configHeaders, 'sortInitialOrder') || c.sortInitialOrder
          );
    ...
    setColumnSort: function (c, $header, isDisabled) {
      ...
      $header[isDisabled ? 'addClass' : 'removeClass']('sorter-false').attr(
        'aria-disabled',
        '' + isDisabled
      );
      ...
      if (c.widgets.length) {
        ...
        for (indx = 0; indx < len; indx++) {
          ...
          if (widget && widget.id) {
            ...
            console.warn(
              '"' + names[indx] + '" was enabled, but the widget code has not been loaded!'
            );
          }
        }
        ...
        if (debug) {
          console[console.group ? 'group' : 'log'](
            'Start ' + (init ? 'initializing' : 'applying') + ' widgets'
          );
        }
        for (indx = 0; indx < len; indx++) {
          widget = widgets[indx];
    ...
    validateOptions: function (c) {
      ...
      if (orig) {
        ...
        for (setting in orig) {
          ...
          if (typ === 'undefined') {
            console.warn(
              'Tablesorter Warning! "table.config.' + setting + '" option not recognized'
            );
          } else if (typ === 'object') {
  ...
  ts.addParser({
    ...
    format: function (str, table) {
      ...
      return str && typeof num === 'number'
        ? num
        : str
        ? $.trim(str && table.config.ignoreCase ? str.toLocaleLowerCase() : str)
        : str;
    },
    type: 'numeric',
  });
