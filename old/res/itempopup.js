...
function createDivCardPopup(data, opts) {
  let containerDiv = $(
    `<div style='position:relative;' class='itemPopupContainer newItemPopup divinationCard'/>`
  );
  if (opts.setId) {
    containerDiv.prop('id', `${data.id}_${data.arrayIndex || ''}_popup`);
  }
  containerDiv.append(
    `<div class='cardFace'><img src='https://web.poecdn.com/image/divination-card/${data.artFilename}.png'/></div>`
  );
...
function getDivCardMods(data) {
  ...
  function parseMod(str) {
    ...
    for (let i = 0; i < markup.length; i++) {
      parsedStr += `<span style='${fontSize ? `font-size:${fontSize / 2}px` : ''}' class='${markup[
        i
      ].replace(/[<>]/g, '')}'>${text[i].replace(/[{}]/g, '')}</span> `;
    }
...
function getDivCardFlavourText(data) {
  ...
  str += `<span class='lc' style='${
    fontSize ? `font-size:${fontSize / 2}px` : ''
  }'>${flavourTextStr}</span>`;
  str += '</div>';
...
function getPopupIcon(data, opts) {
  ...
  return $(`
    <div class='popupIconContainer'>
      <span class='stackSize' style='top:12px;left:18px;z-index:24'>${
        data.maxStackSize ? data.pickupStackSize || data.stackSize : ''
      }</span>
      <div style='position:relative;min-width:${data.w * 47}px;min-height:${
    data.h * 47
  }px;' class='icon newItemContainer iW${data.w} iH${data.h}'>
...
function getHeader(data) {
  ...
  if (data.name) {
    ...
    row.addClass('doubleLine');
    subCell.append(
      $(`<div class='itemName secretName'><span class='lc'>${data.secretName}</span></div>`)
    );
  ...
  if (data.hybrid) {
    subCell.append(
      $(`<div class='itemName typeLine'><span class='lc'>${data.hybrid.baseTypeName}</span></div>`)
    );
    setMaxLength(data.hybrid.baseTypeName);
  } else {
    subCell.append(
      $(`<div class='itemName typeLine'><span class='lc'>${data.typeLine}</span></div>`)
    );
...
function getItemLevelAndRequirements(data) {
  ...
  if (data.ilvl) {
    div.append(
      $(
        `<div class='itemLevel'><span>Item Level: </span><span class='colourDefault'>${data.ilvl}</span></div>`
      )
    );
  }
  ...
  if (data.veiledMods) {
    for (let i = 0; i < data.veiledMods.length; i++) {
      ...
      if (mod.includes('prefix')) {
        div.append(
          $(`<div class='veiledMod'><span class='lc prefix ${mod}'>Veiled Prefix</span></div>`)
        );
      } else if (mod.includes('suffix')) {
        div.append(
          $(`<div class='veiledMod'><span class='lc suffix ${mod}'>Veiled Suffix</span></div>`)
        );
      }
    }
  }
...
function getIncubatedItem(data) {
  ...
  div.append(
    ...
    <div class="incubated">
      ...
      <span class="experienceBar"><span class="fill"><span style="width: ${progress}%;"></span></span></span>
      <div class="descrText"><span><span class="progress">${f.format(i.progress)}/${f.format(
      i.total
    )}</span> Level ${level} Monster Kills</span></div>
    </div>  
  `)
  );
  return div;
