...
async function updateUltimatum() {
  ...
  await DB.all(
    ' select id, firstevent, lastevent, runinfo from mapruns order by id ',
    async (err, rows) => {
      for (let row of rows) {
        DB.all(
              ...
              for (let i = 0; i < events.length; i++) {
                ...
                if (Constants.ultimatumQuotes.start.includes(line.text)) {
                  ...
                  if (Constants.ultimatumQuotes.lost.includes(line.text)) {
                    ...
                    if (
                      currUlt.rounds[evt.id].includes('/') &&
                      currUlt.rounds[evt.id].includes('Ruin')
                    ) {
      ...
      for (let a = 0; a < rows.length; a++) {
          ...
          if (gearSlot.prev && gearSlot.curr) {
            if (Array.isArray(gearSlot.prev)) {
              ...
              if (itemsEqual(gearSlot.prev, gearSlot.curr)) {
                debugLog(
                  `${row.timestamp} : Items in gear slot ${key} not actually different, deleting`
                );
                delete diff[key];
                hasDiff = true;
              }
        ...
        if (hasDiff) {
          ...
          if (str === '{}') {
            ...
            DB.run(
              'update gear set diff = ? where timestamp = ?',
              [JSON.stringify(diff), row.timestamp],
              (err) => {
                if (err) {
...
async function v0286fix() {
  ...
  async function devalueRogueMarkers() {
    ...
    return new Promise(async (resolve, reject) => {
      DB.run(
        " update items set value = 0 where typeline = 'Rogue''s Marker' and value > 0 ",
        function (err) {
          if (err) {
            debugLog(err.toString());
          } else {
  ...
  async function fixConquerorBattleCounts() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let runs = await getMapsSql(
        " select * from mapruns where id > 20210115000000 and runinfo like '%conq%' and runinfo like '%sirus%' "
      );
      let updateStmt = DB.prepare(' update mapruns set runinfo = ? where id = ? ');
      ...
      for (let i = 0; i < runs.length; i++) {
        ...
        if (d) {
          debugLog(
            `Fixed conqueror battle count for map ${i + 1} of ${runs.length} with id ${run.id}.`
          );
        } else {
          debugLog(
            `Error conqueror battle count for map ${i + 1} of ${runs.length} with id ${run.id}.`
          );
        }
      }
  ...
  async function fixZanaBlightedMaps() {
    ...
    return new Promise(async (resolve, reject) => {
      DB.run('begin transaction');
      let runs = await getMapsSql(
        " select * from mapruns where id > 20210115000000 and runinfo like '%blightedMap%' and runinfo like '%Zana%' "
      );
      let updateStmt = DB.prepare(' update mapruns set runinfo = ? where id = ? ');
      ...
      for (let i = 0; i < runs.length; i++) {
        ...
        if (d) {
          debugLog(
            `Fixed Zana blighted mission for map ${i + 1} of ${runs.length} with id ${run.id}.`
          );
        } else {
          debugLog(
            `Error fixing Zana blighted mission for map ${i + 1} of ${runs.length} with id ${
              run.id
            }.`
          );
        }
  ...
  async function getMavenEvents() {
      ...
      for (let j = 0; j < runs.length; j++) {
        ...
        if (obj.maven) {
          if (obj.maven.bossKilled) {
            obj.bossBattle = {};
            obj.bossBattle.time = Utils.getRunningTime(
              obj.maven.firstLine,
              obj.maven.bossKilled,
              's',
              { useGrouping: false }
            );
          ...
          if (d) {
            debugLog(
              `Added Maven events for map ${j + 1} of ${runs.length} with id ${run.id} ${
                obj.bossBattle
                  ? ` - boss killed in ${obj.bossBattle.time} sec ${
                      obj.bossBattle.deaths ? `- ${obj.bossBattle.deaths} deaths` : ''
                    }`
                  : ''
              }`
            );
    ...
    function getBossBattleDeaths(start, end) {
      return new Promise((resolve, reject) => {
        DB.get(
          `select count(1) as count from events where event_type='slain' and id between ? and ? order by id`,
          [start, end],
          async (err, row) => {
            if (err) {
              debugLog(err.toString());
      ...
      if (!ver && ver !== 0) {
        ...
        for (let i = 0; i < leagues.length; i++) {
          ...
          let stashes = await getStashes(
            leagues[i].timestamp,
            i < leagues.length - 1 ? leagues[i + 1].timestamp : null
          );
          if (!stashes) {
            debugLog(`Failed to get stashes from char db for ${leagues[i].league}`);
          } else {
            let stashStmt = leagueDB.prepare(
              ' insert into stashes(timestamp, items, value) values(?, ?, ?) '
            );
          ...
          let rates = await getRates(
            leagues[i].timestamp.substring(0, 8),
            i < leagues.length - 1 ? leagues[i + 1].timestamp.substring(0, 8) : null
          );
          if (!rates) {
